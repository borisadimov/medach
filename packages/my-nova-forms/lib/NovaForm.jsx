...
class NovaForm extends Component{
  ...
  uploadFile(event){
    ...
    if(!(files[0].type === 'image/jpeg' || files[0].type === 'image/png')) {return false}
    S3.upload({files: files}, (error, result) => {
        if (error) {
          console.log("Unable to upload");
        }
        else {
          const currentValues = this.state.currentValues;
          const oldValue = currentValues['body'] || ''
          currentValues['body'] = `${oldValue}
                ...
                <div key={field.name}>
                                 ...
                                 updateCurrentValue={this.updateCurrentValue} />
                  {
                    ...
                    <div className='add-image'
                         style={{margin: '10px 0px 10px 50px', textAlign: 'right'}}
                    >
                      <input type="file" ref='fileInput' className="file_bag"/>
                      <button
                        className="upload"
